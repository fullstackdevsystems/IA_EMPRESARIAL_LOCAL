...
def test_r9_4_enterprise_prompt_compiler_builds_full_plan():
    ...
    try:
        ...
        assert plan["prompt_compiler"]["version"] in {"r9.4","r9.5","r9.5.1","r9.6","r9.7","r9.8","r9.9","r10.1"}
        ...
        assert ("enterprise-prompt-compiler-r9." in plan["planner"] or "enterprise-prompt-compiler-r10.1" in plan["planner"])
    finally:
        if old is None:
            os.environ.pop("IA_DYNAMIC_DASHBOARD_LLM", None)
        else:
...
def test_r9_5_advanced_analytics_payload():
    ...
    try:
        ...
        assert p["prompt_compiler"]["version"] in {"r9.5","r9.5.1","r9.6","r9.7","r9.8","r9.9","r10.1"}
...
def test_r9_7_execution_plan_detects_generic_components():
    ...
    try:
        ...
        assert "enterprise-prompt-compiler-r10.1" in plan["planner"]
    finally:
        if old is None: os.environ.pop("IA_DYNAMIC_DASHBOARD_LLM",None)
        else: os.environ["IA_DYNAMIC_DASHBOARD_LLM"] = old
...
def test_r9_7_warning_and_version_are_consistent():
    ...
    assert '"version":"r10.1"' in src
    assert 'R10.1 compiló un plan de ejecución auditable' in src
...
def test_r9_8_real_xlsx_export_and_plan_ready():
    import inspect
    import dashboard_dynamic as dd
    src = inspect.getsource(dd)
    assert 'id="xlsx">Exportar Excel filtrado' in src
    assert 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' in src
    assert 'function buildXlsx(rows)' in src
    assert 'function zipStore(files)' in src
    assert 'dashboard_filtrado.xlsx' in src


def test_r9_8_versions_are_consistent():
    import inspect
    ...
    assert 'enterprise-prompt-compiler-r10.1' in c
    assert '"version":"r10.1"' in c
    assert '"version": "r10.1"' in p
...
def test_r9_9_versions_are_consistent():
    ...
    assert 'enterprise-prompt-compiler-r10.1' in inspect.getsource(epc)
    assert '"version": "r10.1"' in inspect.getsource(pep)
    assert '8.5.5-r10.1' in inspect.getsource(au)
...
def test_r10_1_executive_dashboard_is_clear_and_referenced():
    ...
    assert 'Cobertura visual:' in src
