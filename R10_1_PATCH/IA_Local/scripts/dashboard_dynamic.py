...
let dateFrom='',dateTo='',clientSearch='',chartTop=10,showAllCharts=false;
...
const sb=document.createElement('div');sb.className='searchbox';sb.innerHTML='<label>Buscar cliente</label><input id="clientSearch" placeholder="Nombre o código">';filterRoot.insertBefore(sb,filterRoot.querySelector('.actions'));const tb=document.createElement('div');tb.className='filter';tb.innerHTML='<label>Top gráficas</label><select id="chartTop"><option selected>10</option><option>20</option><option>50</option><option value="9999">Todos</option></select>';filterRoot.insertBefore(tb,filterRoot.querySelector('.actions'));
...
function shell(c,b,cl='',meta={}){const shown=Number(meta.shown||0),total=Number(meta.total||0),coverage=total?100*shown/total:100,unit=measureUnit(c);return `<article class="card ${cl}"><h2>${esc(c.title||'Gráfica')}</h2><div class="chart-ref"><span class="ref-chip">Métrica: ${esc(c.measure||(c.measures||[]).join(' vs ')||'Conteo')}</span><span class="ref-chip">Cálculo: ${esc(opLabel(c.op||'sum'))}</span><span class="ref-chip">Agrupación: ${esc(c.dimension||'—')}</span><span class="ref-chip">Unidad: ${esc(unit)}</span></div><div class="chart">${b}</div><div class="chart-foot"><span>${shown?`Mostrando ${shown} categorías`:''}</span><span>${total&&shown<total?`Cobertura visual: ${coverage.toLocaleString('es-MX',{maximumFractionDigits:1})}% de categorías`:'Cobertura visual: completa'}</span></div></article>`}
...
function bar(c,rows){
 const all=group(rows,c.dimension,c.measure,c.op||'sum').sort((x,y)=>y.value-x.value),limit=Math.min(chartTop,c.top_n||chartTop),a=all.slice(0,limit),mx=Math.max(1,...a.map(x=>Math.abs(x.value))),hasNeg=a.some(x=>x.value<0),clickable=canFilterDimension(c.dimension),unit=measureUnit(c);
...
function warnings(){const w=[...(P.warnings||[])];if(P.status==='insufficient_data'&&!w.length)w.push('Los datos disponibles no contienen todos los campos necesarios para responder fielmente al prompt.');if(w.length){$('auditoria').hidden=false;$('auditoria').classList.add('is-open');$('warnings').innerHTML=w.map(x=>`<li>${esc(x)}</li>`).join('')}}
