...
base.INDEX_HTML = base.INDEX_HTML.replace(
    ...
    "Analizador Universal Empresarial de Excel / CSV - V8.5.5 R10.1 · Dashboard Dinámico IA",
).replace(
    ...
    "<title>IA Empresarial Local - V8.5.5 R10.1 · Dashboard Dinámico IA</title>",
).replace(
    "<h1>Analizador Universal Empresarial de Excel / CSV</h1>",
    "<h1>Analizador Universal Empresarial de Excel / CSV <span style=\"font-size:14px;background:#dbeafe;color:#1d4ed8;padding:4px 8px;border-radius:999px;vertical-align:middle\">V8.5.5 R10.1</span></h1>",
)
...
def version_info() -> Dict[str, Any]:
    return {"version": "8.5.5-r10.1", "motor": "universal-profesional-memoria-rag", "script": "analizador_universal.py", "reportes": "dashboard HTML dinámico por prompt + PDF BI + Excel analitico", "enterprise_ai": "memoria persistente + RAG + datos estructurados + ContextEngine", "controles": "prompt authority + data contract + calculo deterministico + semantic mapper + aislamiento empresa/usuario"}
