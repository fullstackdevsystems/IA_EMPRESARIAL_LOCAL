...
def compile_enterprise_prompt(
    ...
    out["warnings"] = warnings + [
        "R9.5.1 compiló métricas, filtros, visualizaciones y analítica avanzada reactiva desde el prompt.",
        "Los indicadores financieros principales se calculan con código sobre los datos filtrados; la IA no realiza las sumas base.",
        "Las funciones avanzadas no soportadas todavía por el renderer se omiten sin inventar resultados."
    ]
    out["planner"] = str(out.get("planner") or "validated") + "|enterprise-prompt-compiler-r9.6"
    return out
