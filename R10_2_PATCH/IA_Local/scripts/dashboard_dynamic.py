...
def _semantic_columns(df: pd.DataFrame) -> Dict[str, Optional[str]]:
    ...
    # AMBIGUOUS concepts deliberately return None and are never auto-calculated.
    from semantic_layer import usable_semantic_columns
    return usable_semantic_columns(df)
...
function nlDimension(q,rows){
 ...
 const dims=[
  ...
  [['vendedor','ejecutivo','asesor'],sc('seller'),'Vendedor'],[['zona','region'],sc('zone'),'Zona'],[['categoria'],sc('category'),'Categoría'],
