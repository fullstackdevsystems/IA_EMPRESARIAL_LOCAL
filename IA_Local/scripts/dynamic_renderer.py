...
def build_dynamic_renderer_model(spec: Dict[str, Any], plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "ruleset_version":dict(spec.get("provenance") or {}).get("ruleset_version"),
        "legacy": {
...
(function(){
  try{
    ...
    function distinct(rr,col){return new Set(rr.map(r=>String(r[col]??'').trim()).filter(Boolean)).size}
    function kpiValue(c,rr){if(c.status==='BLOCKED')return null;const cols=c.source_columns||[],op=String((c.execution||{}).operator||'');if(op==='difference_of_sums'&&cols.length>=2)return sum(rr,cols[0])-sum(rr,cols[1]);if(op==='ratio_of_sums'&&cols.length>=2){const den=sum(rr,cols[1]);return den?sum(rr,cols[0])/den:null}if(op==='difference_over_sum'&&cols.length>=3){const den=sum(rr,cols[2]);return den?(sum(rr,cols[0])-sum(rr,cols[1]))/den:null}if(op==='ratio_of_sums_pct'&&cols.length>=2){const den=sum(rr,cols[1]);return den?100*sum(rr,cols[0])/den:null}if(op==='difference_over_sum_pct'&&cols.length>=2){const den=sum(rr,cols[0]);return den?100*(sum(rr,cols[0])-sum(rr,cols[1]))/den:null}if(op==='nunique'&&cols[0])return distinct(rr,cols[0]);if(op==='sum_over_nunique'&&cols.length>=2){const den=distinct(rr,cols[1]);return den?sum(rr,cols[0])/den:null}if(!op&&cols.length===1)return sum(rr,cols[0]);return null}
    function ruleLine(c){const r=c.rule||{};if(!r.rule_id)return '';return `<div class="r13b-source">Regla: ${esc(r.rule_id)} · ${esc(c.formula||'')} · división por cero: N/D</div>`}
    function kpiCard(c,rr){if(c.status==='BLOCKED')return blockedCard(c);const v=kpiValue(c,rr),roleName=String(c.semantic_role||''),kind=c.output_format||(/revenue|cost|profit|freight|price|ticket/.test(roleName)?'currency':/margin/.test(roleName)?'percent':/operations|customers|sellers|products/.test(roleName)?'integer':'number');return `<article class="r13b-card third"><h3>${esc(c.title||roleName)}</h3><span class="r13b-status ${esc(c.status||'SUPPORTED')}">${esc(c.status||'SUPPORTED')}</span><div class="r13b-kpi-value">${fmt(v,kind)}</div><div class="r13b-small">${rr.length.toLocaleString('es-MX')} registros filtrados</div>${sourceLine(c.source_columns||[])}${ruleLine(c)}</article>`}
    ...
    function renderPages(){const rr=rows();model.pages.forEach(page=>{const mount=document.querySelector(`[data-r13b-mount="${CSS.escape(page.id)}"]`);if(!mount)return;if(legacyMode){const derived=(page.components||[]).map(id=>model.components[id]).filter(c=>c&&c.type==='kpi'&&c.status==='DERIVABLE'&&((c.provenance||{}).source==='capability_rule_registry'));mount.innerHTML=derived.length?`<div class="r13b-page-meta" style="grid-column:span 12">Métricas derivadas gobernadas · ${esc(model.ruleset_version||'r10.13c')}</div>`+derived.map(c=>kpiCard(c,rr)).join(''):'';}else mount.innerHTML=pageHtml(page,rr)});const planner=$('planner');if(planner)planner.textContent=`R10.13C · ${model.domain||'generic'} · ${model.pages.length} páginas`;}
  ...
  }catch(err){console.error('R10.13C capability-rule renderer fallback:',err)}
})();
</script>
'''
