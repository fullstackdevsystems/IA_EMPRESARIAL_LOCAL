...
def _fallback_plan(df: pd.DataFrame, prompt: str, filename: str, sheet: str, semantic_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        'subtitle': filename + (f' Â· Hoja {sheet}' if sheet else ''),
        'kpis': kpis[:10],
        'charts': charts[:8],
        'filters': filters,
...
<title>__TITLE__ Â· Primos & Cousins</title>
<style>
...
<nav class="nav">
<a class="active" href="#resumen"><span class="ico">â–£</span><span>Resumen</span></a>
<a href="#graficas"><span class="ico">âŒ</span><span>Ventas</span></a>
<a href="#analitica"><span class="ico">â—«</span><span>Rentabilidad</span></a>
<a href="#componentes"><span class="ico">â‡„</span><span>LogÃ­stica</span></a>
<a href="#analitica"><span class="ico">â–¦</span><span>AnÃ¡lisis avanzado</span></a>
<a href="#detalle"><span class="ico">â‰¡</span><span>Detalle</span></a>
<a href="#pregunta"><span class="ico">?</span><span>Preguntar</span></a>
<a href="#auditoria"><span class="ico">âœ“</span><span>Datos y validaciÃ³n</span></a>
</nav>
<div class="side-bottom"><div class="ai-badge"><strong>IA EMPRESARIAL LOCAL</strong>AnÃ¡lisis inteligente de datos</div></div>
</aside>
<main class="main">
<header class="top"><div class="titlebox"><h1 id="title">Dashboard Ejecutivo</h1><div id="subtitle" class="subtitle"></div></div><div class="meta-row"><div class="meta-chip"><span class="k">Archivo</span><span id="filemeta" class="v">â€”</span></div><div class="meta-chip"><span class="k">Registros</span><span id="rowsmeta" class="v">â€”</span></div><div class="meta-chip"><span class="k">Planificador</span><span id="planner" class="v">â€”</span></div></div></header>
<section id="resumen"><div class="toolbar" id="filters"><div class="actions" id="descargas"><button class="btn" id="toggleFilters">MÃ¡s filtros</button><button class="btn" id="clear">Limpiar filtros</button><button class="btn" id="csv">Exportar CSV filtrado</button><button class="btn" id="xlsx">Exportar Excel filtrado</button></div></div><div class="section-head"><div><h2>Indicadores principales</h2><div class="small">Seis KPIs ejecutivos; los indicadores adicionales quedan disponibles bajo demanda.</div></div><button class="btn section-toggle" id="toggleKpis">Ver todos los indicadores</button></div><section class="kpis" id="kpis"></section></section>
<div class="section-head"><div><h2>Lectura ejecutiva</h2><div class="small">GrÃ¡ficas principales con valores, unidad, cobertura y referencia de cÃ¡lculo.</div></div><button class="btn section-toggle" id="toggleCharts">Ver todas las grÃ¡ficas</button></div><section class="grid" id="graficas"><div id="charts" style="display:contents"></div></section>
<section class="audit view-secondary" id="auditoria" hidden><h2>Datos y alertas del anÃ¡lisis</h2><ul id="warnings"></ul></section>
...
<article class="adv-card"><h2>Clientes Â· Rentabilidad</h2><div id="clientsAdvanced"></div></article>
<article class="adv-card"><h2>Productos Â· Rentabilidad</h2><div id="productsAdvanced"></div></article>
<article class="adv-card full"><h2>Operaciones con Utilidad Negativa</h2><div id="negativeAdvanced"></div></article>
<article class="adv-card full"><h2>Rutas Origen â†’ Destino</h2><div id="routesAdvanced"></div></article>
<article class="adv-card full"><h2>ValidaciÃ³n MatemÃ¡tica</h2><div class="findings" id="validationAdvanced"></div></article>
<article class="adv-card full"><h2>Mapa SemÃ¡ntico R10.2</h2><div id="semanticMap"></div></article>
<article class="adv-card full"><h2>Cobertura del Prompt</h2><div id="promptCoverage"></div></article>
</section>
<section class="nl-card view-secondary" id="pregunta"><div class="nl-head"><div><h2>PregÃºntale al Dashboard</h2><div class="small">Consulta en lenguaje natural sobre la selecciÃ³n actual. Los cÃ¡lculos se hacen de forma determinÃ­stica sobre los datos filtrados.</div></div><span class="nl-badge">R10.2 Â· Consulta local</span></div><div class="nl-form"><input id="nlQuestion" type="text" placeholder="Ej. Â¿CuÃ¡l fue el producto con mayor utilidad?"><button class="btn" id="nlAsk">Analizar pregunta</button></div><div class="nl-examples"><button class="nl-example">Â¿CuÃ¡l fue el producto con mayor utilidad?</button><button class="nl-example">Â¿QuÃ© cliente tuvo mayor venta?</button><button class="nl-example">Â¿CuÃ¡l es la utilidad total?</button><button class="nl-example">Top 5 vendedores por toneladas</button></div><div class="nl-result" id="nlResult"><div class="nl-answer">Escribe una pregunta sobre ventas, utilidad, costos, toneladas, fletes, clientes, productos, vendedores, zonas, categorÃ­as, proveedores o almacenes.</div></div></section>
...
<footer class="footer"><span><strong>PRIMOS & COUSINS</strong> Â· Innovando Juntos</span><span>Dashboard generado automÃ¡ticamente por IA Empresarial Local</span></footer>
</main></div>
<script>
...
const sb=document.createElement('div');sb.className='searchbox';sb.innerHTML='<label>Buscar cliente</label><input id="clientSearch" placeholder="Nombre o cÃ³digo">';filterRoot.insertBefore(sb,filterRoot.querySelector('.actions'));const tb=document.createElement('div');tb.className='filter filter-extra';tb.innerHTML='<label>Top grÃ¡ficas</label><select id="chartTop"><option selected>10</option><option>20</option><option>50</option><option value="9999">Todos</option></select>';filterRoot.insertBefore(tb,filterRoot.querySelector('.actions'));
...
function opLabel(op){return ({sum:'Suma',avg:'Promedio',count:'Conteo',nunique:'Ãšnicos'}[op]||String(op||'Suma'))}
function shell(c,b,cl='',meta={}){const shown=Number(meta.shown||0),total=Number(meta.total||0),unit=measureUnit(c),sv=Number(meta.shownValue),tv=Number(meta.totalValue),hasValue=Number.isFinite(sv)&&Number.isFinite(tv)&&tv!==0,valueCoverage=hasValue?100*sv/tv:100,coverageLabel=meta.coverageLabel||'Cobertura del valor';return `<article class="card ${cl}"><h2>${esc(c.title||'GrÃ¡fica')}</h2><div class="chart-ref"><span class="ref-chip">MÃ©trica: ${esc(c.measure||(c.measures||[]).join(' vs ')||'Conteo')}</span><span class="ref-chip">CÃ¡lculo: ${esc(opLabel(c.op||'sum'))}</span><span class="ref-chip">AgrupaciÃ³n: ${esc(c.dimension||'â€”')}</span><span class="ref-chip">Unidad: ${esc(unit)}</span><span class="ref-chip">Top: ${shown||'Todos'} de ${total||shown||0}</span></div><div class="chart">${b}</div><div class="chart-foot"><span>${shown?`Mostrando Top ${shown} de ${total} categorÃ­as`:''}</span><span class="coverage-strong">${hasValue?`${esc(coverageLabel)}: ${valueCoverage.toLocaleString('es-MX',{maximumFractionDigits:1})}% Â· mostrado ${fmt(sv)} / total ${fmt(tv)}`:'Cobertura del valor: no aplica'}</span></div></article>`}
...
$('toggleFilters').onclick=()=>{showExtraFilters=!showExtraFilters;filterRoot.classList.toggle('show-extra-filters',showExtraFilters);$('toggleFilters').textContent=showExtraFilters?'Menos filtros':'MÃ¡s filtros'};
...
function advGroup(rows,dims){
 ...
 return [...m.entries()].map(([key,rr])=>{
   ...
   o.MargenPct=venta?100*util/venta:null;o.UtilidadTon=ton?util/ton:null;o.FleteTon=ton?flete/ton:null;o.Estado=util<0?'PÃ©rdida':util>0?'Rentable':'Sin utilidad';
...
function dynamicFindings(rows,clients,products,routes,neg){
 ...
 const xs=[
   `SelecciÃ³n actual: ${rows.length.toLocaleString('es-MX')} registros y ${fmt(ton)} toneladas.`,
 ...
 if(routes.length){
   ...
   xs.push(`Ruta con mayor flete acumulado: ${rt.Ciudad_Origen} â†’ ${rt.Ciudad_Destino} (${fmt(rt.Flete,'currency')}).`)
 }
...
function dynamicValidation(rows){
 ...
 return [
  ...
  {name:'Clientes Ãºnicos por Cod_Cliente',ok:true,detail:`${new Set(rows.map(r=>r.Cod_Cliente).filter(v=>v!==null&&v!==undefined&&String(v)!=='')).size} clientes Ãºnicos`},
  {name:'Fechas vÃ¡lidas',ok:badDates===0,detail:`${badDates} fechas invÃ¡lidas`},
  {name:'NaN/Infinity no visibles',ok:true,detail:'Divisiones protegidas y valores seguros.'}
 ]
}
function renderSemanticMap(){
 const sm=P.semantic_map||{},cs=sm.concepts||{},rows=Object.entries(cs).map(([key,x])=>({Concepto:x.label||key,Columna:x.column||'â€”',Confianza:x.confidence||'MISSING',Puntaje:Number(x.score||0),Motivo:x.reason||''}));
 ...
 $('semanticMap').innerHTML=`<div class="note">PolÃ­tica: ${esc(sm.policy||'name+dtype+content+context; ambiguous=no-auto-calc')}</div>`+metricTable(rows,['Concepto','Columna','Confianza','Puntaje','Motivo'],100)
}
function promptCoverage(){
 ...
 $('promptCoverage').innerHTML=`<div class="coverage-score"><b>Cobertura determinÃ­stica: ${fmt(pct,'percent')}</b><div class="coverage-meter"><i style="width:${Math.max(0,Math.min(100,pct))}%"></i></div><span>${Number(ep.ready_count||0)}/${Number(ep.requested_count||0)} listos</span></div>`+
 (items.length?`<div class="coverage-grid">${items.map(x=>`<div class="coverage-item"><b>${esc(x.name)} Â· <span class="${cls(x)}">${label(x)}</span></b>${esc(x.detail||'')}${(x.missing||[]).length?`<br><span class="neg">Falta: ${esc(x.missing.join(', '))}</span>`:''}</div>`).join('')}</div>`:'<div class="note">No se detectaron requisitos explÃ­citos.</div>')
}
...
function opportunityCards(rows){
 ...
 if(neg.length)out.push(`<div class="finding"><b>Operaciones con pÃ©rdida</b><br>${neg.length.toLocaleString('es-MX')} operaciones Â· ${fmt(negImpact,'currency')}</div>`);
 const worst=(a)=>[...a].sort((x,y)=>x.Utilidad-y.Utilidad)[0],best=(a)=>[...a].sort((x,y)=>y.Utilidad-x.Utilidad)[0];
 if(clients.length){const b=best(clients),w=worst(clients);out.push(`<div class="finding"><b>Cliente mÃ¡s rentable</b><br>${esc(b.Cliente??b.Cod_Cliente)} Â· ${fmt(b.Utilidad,'currency')}</div>`);out.push(`<div class="finding"><b>Cliente de mayor riesgo</b><br>${esc(w.Cliente??w.Cod_Cliente)} Â· ${fmt(w.Utilidad,'currency')}</div>`)}
 if(products.length){const b=best(products),w=worst(products);out.push(`<div class="finding"><b>Producto con mayor utilidad</b><br>${esc(b.ctrl_alm??b.Articulo)} Â· ${fmt(b.Utilidad,'currency')}</div>`);out.push(`<div class="finding"><b>Producto con menor utilidad</b><br>${esc(w.ctrl_alm??w.Articulo)} Â· ${fmt(w.Utilidad,'currency')}</div>`)}
 if(sellers.length){const w=worst(sellers);out.push(`<div class="finding"><b>Vendedor con menor utilidad</b><br>${esc(w.Vendedor)} Â· ${fmt(w.Utilidad,'currency')}</div>`)}
 if(zones.length){const b=best(zones),w=worst(zones);out.push(`<div class="finding"><b>Zonas</b><br>Mayor: ${esc(b.Zona)} Â· ${fmt(b.Utilidad,'currency')}<br>Menor: ${esc(w.Zona)} Â· ${fmt(w.Utilidad,'currency')}</div>`)}
...
function renderDynamicComponents(rows){
 ...
 if(execRequested('pivot_customer')){
   ...
   add('TD Â· Resumen por Cliente',metricTable(cs,['Cod_Cliente','Cliente','Toneladas','Venta','Costo','Utilidad','MargenPct','UtilidadTon','Operaciones'],50),true)
 }
 ...
 const ranks=[['sellers','Vendedores',sc('seller','Vendedor')],['zones','Zonas',sc('zone','Zona')],['categories','CategorÃ­as',sc('category','Categoria')],['providers','Proveedores',sc('supplier','Proveedor')],['warehouses','Almacenes',sc('warehouse','Almacen')]];
...
function renderAdvanced(rows){
 ...
 $('negativeAdvanced').innerHTML=`<div class="note">${neg.length.toLocaleString('es-MX')} operaciones negativas en la selecciÃ³n actual.</div>`+metricTable(neg,['Fecha','Refer','Cliente','Articulo','Toneladas_Vendidas','Importe_Venta','Costo','Utilidad','Proveedor','Vendedor'],40);
 ...
 $('validationAdvanced').innerHTML=dynamicValidation(rows).map(x=>`<div class="finding">${x.ok?'âœ“':'âš '} <b>${esc(x.name)}</b><br>${esc(x.detail)}</div>`).join('');renderSemanticMap();promptCoverage()
}
...
function nlDimension(q,rows){
 ...
 const dims=[
  ...
  [['proveedor'],sc('supplier'),'Proveedor'],[['almacen','bodega'],sc('warehouse'),'AlmacÃ©n'],[['semana'],sc('week'),'Semana'],[['fecha','dia'],sc('date'),'Fecha']
 ];
...
function answerNaturalQuestion(question,rows){
 const q=nlNorm(question);if(!q)return {text:'Escribe una pregunta para analizar los datos visibles.',table:''};if(!rows.length)return {text:'No hay registros en la selecciÃ³n actual.',table:''};
 const m=nlMeasure(q);if(!m.col)return {text:'No existe una columna semÃ¡ntica confiable para esa mÃ©trica.',table:''};const dim=nlDimension(q,rows),entity=nlFindEntity(q,rows),isMin=/\b(menor|menos|peor|minimo|minima)\b/.test(q),isMax=/\b(mayor|mas|mejor|maximo|maxima|top)\b/.test(q),topMatch=q.match(/\btop\s*(\d{1,2})\b/),topN=Math.max(1,Math.min(20,topMatch?Number(topMatch[1]):5));
 if(entity){const rr=rows.filter(r=>String(r[entity.col]??'')===entity.value),v=nlAgg(rr,m);return {text:`${entity.value}: ${m.label} = ${nlFmt(v,m)} sobre ${rr.length.toLocaleString('es-MX')} registros de la selecciÃ³n actual.`,table:''}}
...
function openDrill(kind,value){
 ...
 const title=value;$('drillTitle').textContent='Detalle Â· '+title;
