...
Operaciones KPI permitidas: sum, avg, min, max, count, nunique, sum_div_nunique, ratio_pct, difference_sum, variation_pct.
...
def _validate_plan(plan: Optional[Dict[str, Any]], df: pd.DataFrame, fallback: Dict[str, Any]) -> Dict[str, Any]:
    ...
    allowed_ops = {'sum', 'avg', 'min', 'max', 'count', 'nunique', 'sum_div_nunique', 'ratio_pct', 'difference_sum', 'variation_pct'}
    vk = []
...
function kval(k,rows){const vals=c=>rows.map(r=>num(r[c])),op=k.op;if(op==='count')return rows.length;if(op==='nunique')return new Set(rows.map(r=>r[k.column]).filter(v=>v!==null&&v!==undefined&&String(v)!=='')).size;if(['sum','avg','min','max'].includes(op)){const a=vals(k.column);if(!a.length)return 0;if(op==='sum')return a.reduce((x,y)=>x+y,0);if(op==='avg')return a.reduce((x,y)=>x+y,0)/a.length;if(op==='min')return Math.min(...a);return Math.max(...a)}if(op==='sum_div_nunique'){const a=vals(k.numerator).reduce((x,y)=>x+y,0),b=new Set(rows.map(r=>r[k.denominator]).filter(v=>v!==null&&v!==undefined&&String(v)!=='')).size;return b?a/b:0}if(op==='ratio'){const a=vals(k.numerator).reduce((x,y)=>x+y,0),b=vals(k.denominator).reduce((x,y)=>x+y,0);return b?a/b:0}if(op==='ratio_pct'){const a=vals(k.numerator).reduce((x,y)=>x+y,0),b=vals(k.denominator).reduce((x,y)=>x+y,0);return b?100*a/b:0}if(op==='difference_sum')return vals(k.left).reduce((x,y)=>x+y,0)-vals(k.right).reduce((x,y)=>x+y,0);if(op==='variation_pct'){const a=vals(k.current).reduce((x,y)=>x+y,0),b=vals(k.previous).reduce((x,y)=>x+y,0);return b?100*(a-b)/Math.abs(b):0}return 0}
