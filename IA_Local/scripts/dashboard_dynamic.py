...
const MAIN_FILTERS=new Set(['Zona','Vendedor','ctrl_alm']);for(const f of(P.filters||[])){if(f.column==='Fecha')continue;const box=document.createElement('div');box.className='filter'+(MAIN_FILTERS.has(f.column)?'':' filter-extra');const vals=[...new Set(ALL.map(r=>r[f.column]).filter(v=>v!==null&&v!==undefined&&String(v)!==''))].sort((a,b)=>String(a).localeCompare(String(b),'es'));const multi=['Zona','Categoria','Vendedor','ctrl_alm','Proveedor'].includes(f.column);const allOption=multi?'<option value="__ALL__" selected>Todos</option>':'<option value="">Todos</option>';box.innerHTML=`<label>${esc(f.label||f.column)}</label><select data-col="${esc(f.column)}" ${multi?'multiple':''}>${allOption}${vals.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('')}</select>`;filterRoot.insertBefore(box,filterRoot.querySelector('.actions'))}
