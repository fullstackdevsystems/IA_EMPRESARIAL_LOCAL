...
<body><div class="wrap"><div class="card"><div class="head"><div><h2 style="margin:0">Asistente Empresarial Local <span class="badge">V8.5.5 Base Productiva + Streaming</span></h2><div style="color:#64748b">Memoria persistente, documentos fundamentados y cálculos determinísticos.</div></div><div class="links"><a href="/">Analizador</a><a id="admin" href="/admin">Administración</a><button id="logout" class="btn" style="display:none" onclick="enterpriseLogout()">Cerrar sesión</button></div></div><div id="auth" class="warn" style="display:none"></div><div id="msgs" class="msgs"></div><div class="row"><textarea id="q" placeholder="Pregunta sobre documentos, reglas del negocio o datasets..."></textarea><button class="btn" id="send">Enviar</button><button class="btn stop" id="stop">Detener</button></div></div></div>
<script>
let token=sessionStorage.getItem('iaEnterpriseSession')||'',currentUser=null;document.getElementById('admin').href='/admin';
function clearEnterpriseSession(){token='';currentUser=null;sessionStorage.removeItem('iaEnterpriseSession');if(activeController){activeController.abort();activeController=null}document.getElementById('logout').style.display='none';document.getElementById('q').disabled=true;document.getElementById('send').disabled=true}
...
document.getElementById('send').onclick=send;document.getElementById('stop').onclick=stopGeneration;document.getElementById('q').addEventListener('keydown',e=>{if(e.ctrlKey&&e.key==='Enter')send()});if(token){establishSession()}else{showLogin()}
</script></body></html>
"""
