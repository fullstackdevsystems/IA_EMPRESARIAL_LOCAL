...
def _facts(df,clients,products,routes,neg):
    ...
    f={"registros":int(len(df)),"toneladas":ton,"venta":venta,"costo":costo,"utilidad":util,
       ...
       "operaciones_negativas":int((pd.to_numeric(df["Utilidad"],errors="coerce").fillna(0.0)<0).sum()) if "Utilidad" in df.columns else 0,"impacto_negativo":float(pd.to_numeric(df.loc[pd.to_numeric(df["Utilidad"],errors="coerce").fillna(0.0)<0,"Utilidad"],errors="coerce").fillna(0.0).sum()) if "Utilidad" in df.columns else 0.0}
    if clients:
...
def _det_summary(f):
    ...
    out.append(f"Se detectaron {f.get('operaciones_negativas',0)} operaciones con utilidad negativa, con impacto acumulado de {cur(f.get('impacto_negativo',0.0))}.")
