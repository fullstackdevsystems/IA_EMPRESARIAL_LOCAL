...
METRIC_LEXICON = {
    ...
    "profit_per_unit": ("utilidad por tonelada","utilidad por unidad","profit per unit","profit per ton"),
    "price_per_unit": ("precio por tonelada","precio promedio por tonelada","precio por unidad","unit price"),
    "cost_per_unit": ("costo por tonelada","costo por unidad","cost per unit"),
...
def _extract_explicit_pages(raw: str) -> List[Dict[str, str]]:
    ...
    for i, line in enumerate(lines):
        n = norm(line)
        if ("siguientes paginas" in n or "paginas siguientes" in n or "quiero las paginas" in n) and (":" in line or "paginas" in n):
            start = i + 1
            break
    if start is None:
        return []
...
def _detect_domain(text: str) -> Dict[str, Any]:
    ...
    for domain, phrases in DOMAIN_LEXICON.items():
        ...
        scores[domain] = {"score": len(hits), "hits": hits}
    ranked = sorted(scores.items(), key=lambda kv: (-kv[1]["score"], kv[0]))
    domain = ranked[0][0] if ranked and ranked[0][1]["score"] else "generic"
    return {"domain": domain, "scores": scores}

def _detected_keys(text: str, lexicon: Dict[str, Iterable[str]]) -> List[str]:
    return [k for k, phrases in lexicon.items() if _contains(text, phrases)]

NEGATION_CUES = (
    "no generes", "no generar", "no genere", "no incluir", "no incluyas",
    "no quiero", "sin incluir", "excluir", "excluye", "evita", "evitar",
    "no necesito", "no se necesitan", "no son necesarias", "no son necesarios",
)

def _segments(raw: str) -> List[str]:
    # Preserve line/list boundaries because enterprise prompts frequently express
    # explicit exclusions as bullets or short sentences.
    parts = re.split(r"[\r\n]+|(?<=[.!?;])\s+", str(raw or ""))
    return [norm(p) for p in parts if norm(p)]

def _negated_keys(raw: str, lexicon: Dict[str, Iterable[str]]) -> List[str]:
    excluded: List[str] = []
    for segment in _segments(raw):
        ...
        for key, phrases in lexicon.items():
            ...
            if not _contains(segment, phrases):
                continue
            # A specific negative commercial concept must not erase a broader
            # dimension that is positively required elsewhere. Example:
            # "no clientes perdidos" must not suppress customer semantics used
            # by "clientes que recogen".
            if key == "customer" and ("clientes perdidos" in segment or "cliente perdido" in segment or "perfil de cliente" in segment):
                explicit_customer_exclusion = bool(re.search(r"(?:paginas?\s+de\s+|excluir\s+|sin\s+)(?:los\s+|las\s+)?clientes?(?:\s*[,;]|\s+(?:ni|y)\s+|$)", segment))
                if not explicit_customer_exclusion:
                    continue
            excluded.append(key)
    return excluded

def _positive_keys(raw: str, text: str, lexicon: Dict[str, Iterable[str]]) -> tuple[List[str], List[str]]:
    excluded = _negated_keys(raw, lexicon)
    detected = _detected_keys(text, lexicon)
    return [k for k in detected if k not in excluded], excluded

def parse_prompt_intelligence(prompt: str) -> Dict[str, Any]:
    raw = str(prompt or "")
    text = norm(raw)
    domain_info = _detect_domain(text)
    metrics, excluded_metrics = _positive_keys(raw, text, METRIC_LEXICON)
    # Contextual shadowing: "costo total de flete" asks about freight cost, not
    # the generic enterprise cost KPI. Keep generic cost only when it is requested
    # in a segment that is not freight-specific.
    if "cost" in metrics and "freight" in metrics:
        cost_segments = [seg for seg in _segments(raw) if _contains(seg, METRIC_LEXICON["cost"])]
        freight_phrases = METRIC_LEXICON["freight"]
        if cost_segments and all(_contains(seg, freight_phrases) for seg in cost_segments):
            metrics = [m for m in metrics if m != "cost"]
    dimensions, excluded_dimensions = _positive_keys(raw, text, DIMENSION_LEXICON)
    analyses, excluded_analyses = _positive_keys(raw, text, ANALYSIS_LEXICON)
    # A monthly logistics movement is already a specialized temporal trend. Do
    # not double-count/render the generic trend capability for the same request.
    if domain_info["domain"] == "logistics" and "monthly_movement" in analyses and "trend" in analyses:
        analyses = [a for a in analyses if a != "trend"]
    deliverables, excluded_deliverables = _positive_keys(raw, text, DELIVERABLE_LEXICON)
    tables, excluded_tables = _positive_keys(raw, text, TABLE_LEXICON)
    pages, excluded_pages = _positive_keys(raw, text, PAGE_LEXICON)
    explicit_pages = _extract_explicit_pages(raw)
    if explicit_pages:
        pages = [p["id"] for p in explicit_pages]
        excluded_page_set = set(excluded_pages)
        explicit_pages = [p for p in explicit_pages if p["id"] not in excluded_page_set]
        pages = [p["id"] for p in explicit_pages]

    filters: Set[str] = set()
    if _contains(text, ("filtro","filtrar","filtros globales","fecha inicial","fecha final")):
        for role in ("date","week","customer","product","product_group","line","seller","zone","supplier","warehouse"):
            if role in dimensions:
                filters.add(role)
    if _contains(text, ("fecha inicial","fecha final","desde","hasta")):
        filters.add("date")

    rankings = []
    if "ranking" in analyses:
        rankings = [r for r in ("customer","product","seller","line","zone","supplier") if r in dimensions]
    drilldowns = []
    if _contains(text, ("drill down","drill-down","expansion de registros","expansión de registros","perfil individual")):
        drilldowns = [x for x in ("customer","product","seller","line","invoice") if x in dimensions]
    if not deliverables and raw.strip():
        deliverables = ["summary"]

    requested_items = []
    for kind, items in (("metric",metrics),("dimension",dimensions),("analysis",analyses),("table",tables),("filter",sorted(filters)),("deliverable",deliverables)):
        for item in items:
            requested_items.append({"kind":kind,"key":item})
