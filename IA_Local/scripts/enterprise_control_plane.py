...
class EnterpriseControlPlane:
    ...
    def _release(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {key: data.get(key) for key in ("product_version", "release", "channel")}
        except Exception:
            return {"product_version": None, "release": None, "channel": None}
    ...
    def overview(self, principal: Any, *, health: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        provider = self.platform.tenant_config(actor["tenant_id"]).get("ai_provider")
    ...
    def ai_for(self, principal: Any) -> Dict[str, Any]:
        actor = self._permit(principal, "config:read")
        return self._public_provider(self.platform.tenant_config(actor["tenant_id"]).get("ai_provider"))
