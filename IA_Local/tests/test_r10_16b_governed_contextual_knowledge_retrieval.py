...
with tempfile.TemporaryDirectory() as td:
    ...
    p.write_text(json.dumps({"schema_version":"r10.16a","registry_id":"demo","knowledge_version":"1","entries":[
        {"entry_id":"demo.freight.policy.v1","type":"policy","status":"APPROVED","title":"Política de flete","content":"El costo de flete requiere una regla empresarial aprobada.","keywords":["flete","costo","regla"],"scope":{"company_id":"DEMO"},"effective_from":"2026-01-01","provenance":{"source":"manual_validation","document_id":"POL-001"}},
        {"entry_id":"demo.currency.v1","type":"definition","status":"APPROVED","title":"Moneda operativa","content":"MXN","scope":{"company_id":"DEMO"},"provenance":{"source":"manual_validation"}},
        {"entry_id":"demo.freight.draft.v1","type":"fact","status":"DRAFT","title":"Flete borrador","content":"No debe recuperarse","scope":{"company_id":"DEMO"},"provenance":{"source":"draft"}}
    ]}),encoding="utf-8")
    ...
    w=retrieve_contextual_enterprise_knowledge(prompt="costo de flete",registry=reg,context={"company_id":"OTHER"},as_of="2026-09-02")
    check("cross_company_empty",w["matched_entry_count"]==0)
    public=public_knowledge_context(r); check("public_context_has_match",public["matched_entry_count"]==1)
    check("public_context_strips_content","content" not in public["matches"][0])
    wp=Path(td)/"wildcard.json"
    wp.write_text(json.dumps({"schema_version":"r10.16a","registry_id":"bad","knowledge_version":"1","entries":[{"entry_id":"bad.scope.v1","type":"fact","status":"APPROVED","title":"Bad scope","content":"x","scope":{"company_id":"*"},"provenance":{"source":"test"}}]}),encoding="utf-8")
