...
def test_r9_4_enterprise_prompt_compiler_builds_full_plan():
    ...
    try:
        ...
        assert plan["prompt_compiler"]["version"] in {"r9.4","r9.5","r9.5.1","r9.6","r9.7","r9.8"}
...
def test_r9_5_advanced_analytics_payload():
    ...
    try:
        ...
        assert p["prompt_compiler"]["version"] in {"r9.5","r9.5.1","r9.6","r9.7","r9.8"}
...
def test_r9_7_execution_plan_detects_generic_components():
    ...
    try:
        ...
        assert "enterprise-prompt-compiler-r9.8" in plan["planner"]
    finally:
        if old is None: os.environ.pop("IA_DYNAMIC_DASHBOARD_LLM",None)
        else: os.environ["IA_DYNAMIC_DASHBOARD_LLM"] = old
...
def test_r9_7_warning_and_version_are_consistent():
    ...
    assert '"version":"r9.8"' in src
    assert 'R9.8 compiló un plan de ejecución auditable' in src
...
def test_r9_8_versions_are_consistent():
    ...
    assert 'enterprise-prompt-compiler-r9.8' in c
    assert '"version":"r9.8"' in c
    assert '"version": "r9.8"' in p
