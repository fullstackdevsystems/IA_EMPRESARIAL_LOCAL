...
def patch_dynamic(p:Path):
    s=read(p)
    s=replace_once(s,'def _semantic_columns(df: pd.DataFrame) -> Dict[str, Optional[str]]:','def _semantic_columns(df: pd.DataFrame, semantic_context: Optional[Dict[str, Any]] = None) -> Dict[str, Optional[str]]:','dynamic semantic signature')
    # Convert direct return dict to local result and governed merge.
    a=s.find('def _semantic_columns('); b=s.find('\n\ndef _extract_top_n',a)
    if a<0 or b<0: raise RuntimeError('dynamic semantic block')
    block=s[a:b]
    if 'merge_context_roles(result' not in block:
        block=block.replace('    return {\n','    result = {\n',1)
        # append before block end
        block=block.rstrip()+'''\n    try:\n        from enterprise_ai.semantic_registry import merge_context_roles, current_context\n        ctx = semantic_context if semantic_context is not None else current_context()\n        return merge_context_roles(result, ctx)\n    except Exception:\n        return result\n'''
        s=s[:a]+block+s[b:]
    s=s.replace('def _fallback_plan(df: pd.DataFrame, prompt: str, filename: str, sheet: str) -> Dict[str, Any]:','def _fallback_plan(df: pd.DataFrame, prompt: str, filename: str, sheet: str, semantic_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:')
    s=s.replace('    sem = _semantic_columns(df)\n','    sem = _semantic_columns(df, semantic_context)\n',1)
    s=s.replace("def build_dashboard_plan(df: pd.DataFrame, prompt: str, filename: str = '', sheet: str = '') -> Dict[str, Any]:","def build_dashboard_plan(df: pd.DataFrame, prompt: str, filename: str = '', sheet: str = '', semantic_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:")
    s=s.replace('    fallback = _fallback_plan(df, prompt, filename, sheet)\n','    fallback = _fallback_plan(df, prompt, filename, sheet, semantic_context)\n',1)
    s=s.replace("def generate_dynamic_dashboard(output_path: Path, df: pd.DataFrame, prompt: str, filename: str, sheet: str = '') -> Dict[str, Any]:","def generate_dynamic_dashboard(output_path: Path, df: pd.DataFrame, prompt: str, filename: str, sheet: str = '', semantic_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:")
    s=s.replace('    plan = build_dashboard_plan(df, prompt, filename, sheet)\n','    plan = build_dashboard_plan(df, prompt, filename, sheet, semantic_context)\n',1)
    write(p,s)
